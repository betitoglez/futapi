...
class Core {
    ...
    public function bid($trade_id, $bid, $fast = false) {
        ...
        try {
            ...
            file_put_contents(storage_path().'/logs/response.txt',print_r($response, TRUE));
        } catch(FutError $e) {
            return false;
        }
    ...
    public function removeSoldAll() {
        $response = $this->request('DELETE', 'trade/sold');
        return $response;
    }
    ...
    public function watchlistDeleteAll (Array $item_ids){
        $response = $this->request('DELETE', 'watchlist', [], ['tradeId'=>implode(',',$item_ids)] );
        return $response;
    }

    public function sendToTradepile($item_id, $safe = true) {
        if($safe && isset($this->tradepile_size)) {
    ...
    public function request($method, $url, $data = [], $params = [], $delay = true) {
        ...
        switch(strtoupper($method)) {
            ...
            case "POST":
                ...
                break;
            case "PUT":
                $response = $response = $this->client->request('PUT', $url, [
                    'query' => $params,
                    'body' => json_encode($data),
                    'headers' => $this->clientHeaders,
                    'debug' => $this->debug
